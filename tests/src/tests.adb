...
procedure Tests is
   ...
   Status   : AUnit.Status;
begin
   if Ada.Environment_Variables.Exists ("OPENCV_CORE_WINDOWS_CI") then
      declare
         Count_Filter : aliased Windows_CI_Output.Counting_Filter;
      ...
      begin
         ...
         declare
            Total      : constant Natural :=
              Windows_CI_Output.Count (Count_Filter);
            Run_Filter : aliased Windows_CI_Output.Running_Filter (Total);
            Results    : Windows_CI_Output.Concise_Result (Total);
            Summary    : Windows_CI_Output.Summary_Reporter;
            Options    : AUnit.Options.AUnit_Options :=
              AUnit.Options.Default_Options;
         begin
            Ada.Text_IO.Put_Line ("OpenCV Core Ada Windows test run");
