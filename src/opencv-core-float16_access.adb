...
package body OpenCV.Core.Float16_Access is
   ...
   begin
      if Image.Depth /= Float16 then
         Raise_Invalid_Access
           ("Float16 typed Mat access requires a Float16 Mat");
   ...
   function Get (Image : Mat; Row, Column : Integer) return Float16_Value is
   begin
      Validate (Image, Row, Column);
      return
        OpenCV.Core.Internal.Typed_Access.Get_Float16 (Image, Row, Column);
   end Get;

   procedure Set
     (Image : in out Mat; Row, Column : Integer; Value : Float16_Value) is
   begin
      Validate (Image, Row, Column);
      OpenCV.Core.Internal.Typed_Access.Set_Float16
   ...
   function Get (Image : Mat; Indices : Index_Array) return Float16_Value is
   begin
      Validate (Image, Indices);
      return OpenCV.Core.Internal.Typed_Access.Get_Float16 (Image, Indices);
   end Get;

   procedure Set
     (Image : in out Mat; Indices : Index_Array; Value : Float16_Value) is
   begin
      Validate (Image, Indices);
      OpenCV.Core.Internal.Typed_Access.Set_Float16 (Image, Indices, Value);
   end Set;
