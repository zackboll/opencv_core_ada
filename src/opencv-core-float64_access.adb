...
package body OpenCV.Core.Float64_Access is
   ...
   begin
      if Image.Depth /= Float64 then
         Raise_Invalid_Access
           ("Float64 typed Mat access requires a Float64 Mat");
   ...
   function Get (Image : Mat; Row, Column : Integer) return Float64_Value is
   begin
      Validate (Image, Row, Column);
      return
        OpenCV.Core.Internal.Typed_Access.Get_Float64 (Image, Row, Column);
   end Get;
   ...
   is
      Result : aliased OpenCV.Internal.C_API.C_Int32 :=
        OpenCV.Internal.C_API.Float64_Finite;
      Status : OpenCV.Internal.C_API.Status;
   begin
      Validate (Image, Row, Column);
      Status :=
        OpenCV.Internal.C_API.Mat_Classify_Float64
          (Self   => Image.Handle,
           Row    => OpenCV.Internal.C_API.C_Int32 (Row),
           Column => OpenCV.Internal.C_API.C_Int32 (Column),
           Result => Result'Access);
      Raise_On_Error (Status, "Float64 typed Mat classification");

      case Result is
         when OpenCV.Internal.C_API.Float64_Finite            =>
            return Finite;

         when OpenCV.Internal.C_API.Float64_Positive_Infinity =>
            return Positive_Infinity;

         when OpenCV.Internal.C_API.Float64_Negative_Infinity =>
            return Negative_Infinity;

         when OpenCV.Internal.C_API.Float64_Not_A_Number      =>
            return Not_A_Number;

         when others                                          =>
            Raise_Invalid_Access
              ("Float64 typed Mat classification returned an invalid value");
            return Finite;
      end case;
   end Classify;

   procedure Set
     (Image : in out Mat; Row, Column : Integer; Value : Float64_Value) is
   begin
      Validate (Image, Row, Column);
      OpenCV.Core.Internal.Typed_Access.Set_Float64
   ...
   function Get (Image : Mat; Indices : Index_Array) return Float64_Value is
   begin
      Validate (Image, Indices);
      return OpenCV.Core.Internal.Typed_Access.Get_Float64 (Image, Indices);
   end Get;

   procedure Set
     (Image : in out Mat; Indices : Index_Array; Value : Float64_Value) is
   begin
      Validate (Image, Indices);
      OpenCV.Core.Internal.Typed_Access.Set_Float64 (Image, Indices, Value);
   end Set;
