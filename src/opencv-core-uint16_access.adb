...
package body OpenCV.Core.UInt16_Access is
   ...
   begin
      if Image.Depth /= UInt16 then
         Raise_Invalid_Access
           ("UInt16 typed Mat access requires a UInt16 Mat");
   ...
   function Get (Image : Mat; Row, Column : Integer) return UInt16_Value is
   begin
      Validate (Image, Row, Column);
      return OpenCV.Core.Internal.Typed_Access.Get_UInt16 (Image, Row, Column);
   end Get;

   procedure Set
     (Image : in out Mat; Row, Column : Integer; Value : UInt16_Value) is
   begin
      Validate (Image, Row, Column);
      OpenCV.Core.Internal.Typed_Access.Set_UInt16 (Image, Row, Column, Value);
   end Set;
   ...
   function Get (Image : Mat; Indices : Index_Array) return UInt16_Value is
   begin
      Validate (Image, Indices);
      return OpenCV.Core.Internal.Typed_Access.Get_UInt16 (Image, Indices);
   end Get;

   procedure Set
     (Image : in out Mat; Indices : Index_Array; Value : UInt16_Value) is
   begin
      Validate (Image, Indices);
      OpenCV.Core.Internal.Typed_Access.Set_UInt16 (Image, Indices, Value);
   end Set;
