package OpenCV.Core.Float64_Mat_View is
   ...
   --  CV_64FC1 Mat that aliases Data. Shape'Length must be in 2 .. 32 and
   ...
   --  has the same geometry as the Rows/Columns overload. Values are the
   --  stored binary64 elements; nothing is narrowed. No data is copied and
   --  the caller retains ownership. Callback lifetime and escape rules are
   --  identical to the Rows/Columns overload: shallow copies, Slice, and
   --  Reshape are rejected; Clone is the independent escape path.
