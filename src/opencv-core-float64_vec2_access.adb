...
package body OpenCV.Core.Float64_Vec2_Access is
   procedure Validate (Image : Mat) is
   begin
      if Image.Depth /= Float64 or else Image.Channels /= 2 then
         Ada.Exceptions.Raise_Exception
           (OpenCV_Error'Identity, "Float64 Vec2 requires Float64 C2 Mat");
      end if;
   end Validate;
   ...
   function Get (Image : Mat; Row, Column : Integer) return Float64_Vec2.Vector
   is
   begin
      Validate (Image, Row, Column);
      return Internal.Typed_Access.Get_Float64_Vec2 (Image, Row, Column);
   end Get;
   procedure Set
   ...
   begin
      Validate (Image, Row, Column);
      Internal.Typed_Access.Set_Float64_Vec2 (Image, Row, Column, Value);
   end Set;
   function Get (Image : Mat; Indices : Index_Array) return Float64_Vec2.Vector
   is
   begin
      Validate (Image, Indices);
      return Internal.Typed_Access.Get_Float64_Vec2 (Image, Indices);
   end Get;
   procedure Set
   ...
   begin
      Validate (Image, Indices);
      Internal.Typed_Access.Set_Float64_Vec2 (Image, Indices, Value);
   end Set;
