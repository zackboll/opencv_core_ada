...
package body OpenCV.Core.Int32_Row_Access is
   ...
   begin
      if Image.Depth /= Int32 then
         Raise_Invalid_Access ("Int32 row access requires an Int32 Mat");
   ...
   procedure Read_Row (Image : Mat; Row : Natural; Data : out Row_Array) is
      Buffer :
        OpenCV.Core.Internal.Typed_Access.Int32_Row_Buffer (1 .. Data'Length);
   begin
      Validate (Image, Row, Data'Length);
      OpenCV.Core.Internal.Typed_Access.Read_Int32_Row
        (Image, Integer (Row), Buffer);

      for Index in Data'Range loop
         Data (Index) :=
           Int32_Value (Buffer (Index - Data'First + Buffer'First));
      end loop;
   end Read_Row;
   ...
   begin
      ...
      for Index in Data'Range loop
         Buffer (Index - Data'First + Buffer'First) :=
           OpenCV.Internal.C_API.C_Int32 (Data (Index));
      end loop;

      OpenCV.Core.Internal.Typed_Access.Write_Int32_Row
        (Image, Integer (Row), Buffer);
   end Write_Row;
...
end OpenCV.Core.Int32_Row_Access;
