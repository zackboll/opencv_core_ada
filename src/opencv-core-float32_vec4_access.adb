...
package body OpenCV.Core.Float32_Vec4_Access is
   procedure Validate (Image : Mat) is
   begin
      if Image.Depth /= Float32 or else Image.Channels /= 4 then
         Ada.Exceptions.Raise_Exception
           (OpenCV_Error'Identity, "Float32 Vec4 requires Float32 C4 Mat");
      end if;
   end Validate;
   ...
   function Get (Image : Mat; Row, Column : Integer) return Float32_Vec4.Vector
   is
   begin
      Validate (Image, Row, Column);
      return Internal.Typed_Access.Get_Float32_Vec4 (Image, Row, Column);
   end Get;
   ...
   begin
      Validate (Image, Row, Column);
      Internal.Typed_Access.Set_Float32_Vec4 (Image, Row, Column, Value);
   end Set;

   function Get (Image : Mat; Indices : Index_Array) return Float32_Vec4.Vector
   is
   begin
      Validate (Image, Indices);
      return Internal.Typed_Access.Get_Float32_Vec4 (Image, Indices);
   end Get;
   ...
   begin
      Validate (Image, Indices);
      Internal.Typed_Access.Set_Float32_Vec4 (Image, Indices, Value);
   end Set;
