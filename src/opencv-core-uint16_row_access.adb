...
package body OpenCV.Core.UInt16_Row_Access is
   ...
   begin
      if Image.Depth /= UInt16 then
         Raise_Invalid_Access ("UInt16 row access requires a UInt16 Mat");
   ...
   procedure Read_Row (Image : Mat; Row : Natural; Data : out Row_Array) is
      Buffer :
        OpenCV.Core.Internal.Typed_Access.UInt16_Row_Buffer (1 .. Data'Length);
   begin
      Validate (Image, Row, Data'Length);
      OpenCV.Core.Internal.Typed_Access.Read_UInt16_Row
        (Image, Integer (Row), Buffer);

      for Index in Data'Range loop
         Data (Index) :=
           UInt16_Value (Buffer (Index - Data'First + Buffer'First));
      end loop;
   end Read_Row;
   ...
   begin
      ...
      for Index in Data'Range loop
         Buffer (Index - Data'First + Buffer'First) :=
           OpenCV.Internal.C_API.C_UInt16 (Data (Index));
      end loop;

      OpenCV.Core.Internal.Typed_Access.Write_UInt16_Row
        (Image, Integer (Row), Buffer);
   end Write_Row;
...
end OpenCV.Core.UInt16_Row_Access;
