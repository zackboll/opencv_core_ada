...
package OpenCV.Core.Float64_Vec4_Mat_View is
   type Buffer_Array is array (Natural range <>) of Float64_Vec4.Vector;
   ...
   --  (not 4 * product scalars), final dimension fastest. Neither Ada lower
   --  bound is visible. No copy or narrowing; the caller keeps ownership;
   --  shallow escape (including Slice and Reshape) is rejected and Clone is
   --  the independent escape path.
   procedure With_Writable_Mat_View
     (Data    : aliased in out Buffer_Array;
