...
package body OpenCV.Core.Int16_Access is
   ...
   begin
      if Image.Depth /= Int16 then
         Raise_Invalid_Access ("Int16 typed Mat access requires an Int16 Mat");
   ...
   function Get (Image : Mat; Row, Column : Integer) return Int16_Value is
   begin
      Validate (Image, Row, Column);
      return OpenCV.Core.Internal.Typed_Access.Get_Int16 (Image, Row, Column);
   end Get;

   procedure Set
     (Image : in out Mat; Row, Column : Integer; Value : Int16_Value) is
   begin
      Validate (Image, Row, Column);
      OpenCV.Core.Internal.Typed_Access.Set_Int16 (Image, Row, Column, Value);
   end Set;
   ...
   function Get (Image : Mat; Indices : Index_Array) return Int16_Value is
   begin
      Validate (Image, Indices);
      return OpenCV.Core.Internal.Typed_Access.Get_Int16 (Image, Indices);
   end Get;

   procedure Set
     (Image : in out Mat; Indices : Index_Array; Value : Int16_Value) is
   begin
      Validate (Image, Indices);
      OpenCV.Core.Internal.Typed_Access.Set_Int16 (Image, Indices, Value);
   end Set;
