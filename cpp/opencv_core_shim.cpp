...
static_assert(sizeof(float) == 4 && sizeof(cv::Vec<float, 4>) == 16 &&
                  CV_ELEM_SIZE(CV_32FC4) == 16,
              "Float32 C4 must occupy 16 bytes");
...
static_assert(sizeof(opencv_core_float32_vec4) == 16 &&
                  ...
                  offsetof(opencv_core_float32_vec4, component_2) == 8 &&
                  offsetof(opencv_core_float32_vec4, component_3) == 12,
              "Float32 Vec4 ABI layout mismatch");
static_assert(sizeof(opencv_core_float64_vec4) == 32 &&
                  offsetof(opencv_core_float64_vec4, component_0) == 0 &&
                  offsetof(opencv_core_float64_vec4, component_1) == 8 &&
                  offsetof(opencv_core_float64_vec4, component_2) == 16 &&
                  offsetof(opencv_core_float64_vec4, component_3) == 24,
              "Float64 Vec4 ABI layout mismatch");
...
opencv_core_status
prepare_float16_row(const opencv_core_mat_handle *mat, int32_t row,
    ...
    // ABI safety: the shim copies element_count * 2 bytes from a formed
    ...
    // ABI safety: copying two bytes per column from a smaller stored
    // element would read or write past the logical row.
    if (mat->value.elemSize() != sizeof(uint16_t)) {
        return invalid_argument(
            "Mat element size does not match the Float16 row accessor");
    }
    ...
    byte_count = static_cast<std::size_t>(element_count) * sizeof(uint16_t);
    ...
    // bits without aliasing uint16_t as cv::hfloat or cv::float16_t.
...
opencv_core_status
prepare_vector_row(const opencv_core_mat_handle *mat, int32_t row,
                 uint64_t element_count, int expected_depth,
                  const char *depth_name, const char *channel_message,
                  const cv::Vec<T, N> *&row_data) {
    row_data = nullptr;
    ...
    // ABI safety: this helper forms a typed vector row pointer and copies
    // element_count * N scalars. Depth, channel, dimension, and bounds
    ...
    // ABI safety: the shim indexes a row of vector objects and a caller's
    // scalar buffer; their byte extents must fit native size_t arithmetic.
    if (element_count > static_cast<uint64_t>(
                            std::numeric_limits<std::size_t>::max() /
                            (N * sizeof(T)))) {
        return invalid_argument(
            "vector row byte count exceeds the native size range");
    }

    // ABI safety: ptr<T> does not check the element layout or storage before
    // the shim indexes the returned pointer as complete native vectors.
    if (mat->value.elemSize() != sizeof(cv::Vec<T, N>) ||
        (element_count != 0 && mat->value.data == nullptr)) {
        return invalid_argument("Mat has no matching vector row storage");
    }

    row_data = mat->value.template ptr<cv::Vec<T, N>>(static_cast<int>(row));
    return OPENCV_CORE_OK;
}
...
static_assert(sizeof(opencv_core_float16_storage) == sizeof(uint16_t),
              "OpenCV binary16 storage must be exactly 16 bits");

uint16_t float16_bits_from_storage(const opencv_core_float16_storage &value) {
    uint16_t bits = 0;
    std::memcpy(&bits, static_cast<const void *>(&value), sizeof(bits));
    return bits;
}

opencv_core_float16_storage float16_storage_from_bits(uint16_t bits) {
    opencv_core_float16_storage value;
    std::memcpy(static_cast<void *>(&value), &bits, sizeof(bits));
    return value;
}
...
// After Ada compatibility checks, publicly empty operands may still mix a
// default cv::Mat() (dims == 0) with a typed 0x0 Mat (dims == 2). OpenCV 5
// elementwise output creation can then build a 0-D scalar destination and
// throw in getContinuousSize2D. Detect that mix without treating every
// typed 0x0 pair as a special case.
bool is_compatible_empty_mix(const cv::Mat &left,
...
bool is_compatible_empty_mix(const cv::Mat &left, const cv::Mat &right,
    ...
    return left.empty() && right.empty() && mask.empty() &&
           (is_default_empty_mat(left) || is_default_empty_mat(right) ||
            is_default_empty_mat(mask));
}
...
// OpenCV compatibility: construct an independently owned empty result that
// preserves the source header's type. A genuine default cv::Mat() stays a
// genuine default result. A typed empty source (including 0x0) keeps its
// depth and channel count with rows == 0 and columns == 0. Do not alias the
// source header.
opencv_core_status
make_empty_like(const cv::Mat &source, opencv_core_mat_handle **out_mat) {
    if (is_default_empty_mat(source)) {
        return make_default_empty_mat(out_mat);
    }

    *out_mat = new opencv_core_mat_handle(cv::Mat(0, 0, source.type()));
    return OPENCV_CORE_OK;
}
...
opencv_core_status opencv_core_mat_acquire_borrow_lease(
    const opencv_core_mat_handle *source, opencv_core_mat_handle **out_mat) {
    clear_error();
...
opencv_core_mat_bitwise_and(const opencv_core_mat_handle *left,
    ...
    *out_mat = nullptr;
    ...
    try {
        ...
        // totals do not match. After Ada compatibility checks, publicly
        // compatible empty operands, including a default-empty / typed 0x0
        // mix, produce an independently owned empty result.
        if (is_compatible_empty_mix(left->value, right->value)) {
            return make_default_empty_mat(out_mat);
        }
...
opencv_core_mat_bitwise_and_masked(const opencv_core_mat_handle *left,
    ...
    *out_mat = nullptr;
    ...
    try {
        // OpenCV compatibility: OpenCV 5 masked bitwise operations create a
        // 0-D scalar destination for default-constructed operands (dims == 0).
        // getContinuousSize2D then throws because source and destination
        // totals do not match. After Ada compatibility and mask checks,
        // publicly compatible empty operands and masks, including a
        // default-empty / typed 0x0 mix, produce an independently owned empty
        // result whose type follows the operands, not the mask.
        if (is_compatible_empty_mix(left->value, right->value, mask->value)) {
            return make_empty_like(left->value, out_mat);
        }
...
opencv_core_mat_bitwise_not_masked(const opencv_core_mat_handle *source,
    ...
    *out_mat = nullptr;
    ...
    try {
        // OpenCV compatibility: OpenCV 5 masked bitwise_not creates a 0-D
        // scalar destination for a default-constructed source (dims == 0).
        // getContinuousSize2D then throws because source and destination
        // totals do not match. After Ada mask checks, a compatible empty
        // source and mask produce an independently owned empty result whose
        // type follows the source, not the mask.
        if (is_compatible_empty_mix(source->value, mask->value)) {
            return make_empty_like(source->value, out_mat);
        }
...
opencv_core_mat_get_uint16(const opencv_core_mat_handle *mat, int32_t row,
    ...
    *out_value = 0;
    ...
    try {
        const opencv_core_status status =
            validate_typed_at(mat->value, row, column, CV_16U, 1,
                              "Mat depth must be UInt16",
                              "Mat must have exactly one channel");
        if (status != OPENCV_CORE_OK) {
            return status;
        }
        ...
        *out_value =
            mat->value.at<uint16_t>(static_cast<int>(row),
                                    static_cast<int>(column));
        return OPENCV_CORE_OK;
    } catch (...) {
        return translate_current_exception();
...
opencv_core_status
opencv_core_mat_set_uint16(opencv_core_mat_handle *mat, int32_t row,
...
opencv_core_status
opencv_core_mat_get_int16(const opencv_core_mat_handle *mat, int32_t row,
                          int32_t column, int16_t *out_value) {
    clear_error();
    ...
    *out_value = 0;
    ...
    try {
        const opencv_core_status status =
            validate_typed_at(mat->value, row, column, CV_16S, 1,
                              "Mat depth must be Int16",
                              "Mat must have exactly one channel");
        if (status != OPENCV_CORE_OK) {
            return status;
        }
        ...
        *out_value =
            mat->value.at<int16_t>(static_cast<int>(row),
                                   static_cast<int>(column));
        return OPENCV_CORE_OK;
    } catch (...) {
        return translate_current_exception();
...
opencv_core_status
opencv_core_mat_set_int16(opencv_core_mat_handle *mat, int32_t row,
                          int32_t column, int16_t value) {
    clear_error();
...
opencv_core_mat_get_float16(const opencv_core_mat_handle *mat, int32_t row,
    ...
    *out_value = 0;
    ...
    try {
        const opencv_core_status status =
            validate_typed_at(mat->value, row, column, CV_16F, 1,
        ...
        const opencv_core_float16_storage value =
            mat->value.at<opencv_core_float16_storage>(
                static_cast<int>(row), static_cast<int>(column));
        *out_value = float16_bits_from_storage(value);
        return OPENCV_CORE_OK;
    } catch (...) {
        return translate_current_exception();
    }
...
opencv_core_mat_set_float16(opencv_core_mat_handle *mat, int32_t row,
    ...
    try {
        ...
        mat->value.at<opencv_core_float16_storage>(
            static_cast<int>(row), static_cast<int>(column)) =
            float16_storage_from_bits(value);
        return OPENCV_CORE_OK;
    } catch (...) {
        return translate_current_exception();
    }
...
opencv_core_mat_get_uint16_nd(const opencv_core_mat_handle *mat, int32_t ndims,
    ...
    *out_value = 0;
    ...
    try {
        ...
        *out_value = mat->value.at<uint16_t>(opencv_indices);
        return OPENCV_CORE_OK;
    } catch (...) {
        return translate_current_exception();
    }
...
opencv_core_status
opencv_core_mat_set_uint16_nd(opencv_core_mat_handle *mat, int32_t ndims,
    ...
    try {
        ...
        mat->value.at<uint16_t>(opencv_indices) = value;
        return OPENCV_CORE_OK;
    } catch (...) {
        return translate_current_exception();
    }
...
opencv_core_status
opencv_core_mat_get_int16_nd(const opencv_core_mat_handle *mat, int32_t ndims,
                             const int32_t *indices, int16_t *out_value) {
    clear_error();
    ...
    *out_value = 0;
    ...
    try {
        ...
        *out_value = mat->value.at<int16_t>(opencv_indices);
        return OPENCV_CORE_OK;
    } catch (...) {
        return translate_current_exception();
    }
...
opencv_core_status
opencv_core_mat_set_int16_nd(opencv_core_mat_handle *mat, int32_t ndims,
                             const int32_t *indices, int16_t value) {
    clear_error();
    ...
    try {
        ...
        mat->value.at<int16_t>(opencv_indices) = value;
        return OPENCV_CORE_OK;
    } catch (...) {
        return translate_current_exception();
    }
...
opencv_core_mat_get_float16_nd(const opencv_core_mat_handle *mat,
    ...
    *out_value = 0;
    ...
    try {
        ...
        const opencv_core_status status =
            prepare_nd_typed_at(mat->value, ndims, indices, opencv_indices,
                                sizeof(opencv_core_float16_storage));
        if (status != OPENCV_CORE_OK) {
            return status;
        }

        const opencv_core_float16_storage value =
            mat->value.at<opencv_core_float16_storage>(opencv_indices);
        *out_value = float16_bits_from_storage(value);
        return OPENCV_CORE_OK;
    } catch (...) {
        return translate_current_exception();
...
opencv_core_mat_read_uint16_row(const opencv_core_mat_handle *mat, int32_t row,
    ...
    try {
        const uint16_t *row_data = nullptr;
        std::size_t byte_count = 0;
        const opencv_core_status status =
            prepare_row(mat, row, element_count, CV_16U,
                        "Mat depth must be UInt16", row_data, byte_count);
        if (status != OPENCV_CORE_OK) {
            return status;
        }
...
opencv_core_status
opencv_core_mat_write_uint16_row(opencv_core_mat_handle *mat, int32_t row,
    ...
    try {
        ...
        if (byte_count != 0) {
            std::memcpy(const_cast<uint16_t *>(row_data), data, byte_count);
        }
        return OPENCV_CORE_OK;
    } catch (...) {
        return translate_current_exception();
...
opencv_core_mat_read_uint8_vec3_row(const opencv_core_mat_handle *mat,
    ...
    try {
        ...
        const opencv_core_status status =
            prepare_vector_row(mat, row, element_count, CV_8U,
                               "Mat depth must be UInt8",
...
opencv_core_mat_read_float32_vec3_row(const opencv_core_mat_handle *mat,
    ...
    try {
        ...
        const opencv_core_status status =
            prepare_vector_row(mat, row, element_count, CV_32F,
                               "Mat depth must be Float32",
                               "Mat must have exactly three channels", row_data);
        if (status != OPENCV_CORE_OK) {
            return status;
...
opencv_core_mat_read_float64_vec3_row(const opencv_core_mat_handle *mat,
    ...
    try {
        ...
        const opencv_core_status status = prepare_vector_row(
...
extern "C++" {
template <typename T, typename Abi>
opencv_core_status vec4_get(const opencv_core_mat_handle *mat, int32_t row,
                            int32_t column, Abi *out, int depth,
                            const char *message) {
    clear_error();
    if (out == nullptr) return invalid_argument("out_value must not be null");
    *out = {0, 0, 0, 0};
    ...
    try {
        const auto status = validate_typed_at(
            mat->value, row, column, depth, 4, message,
            "Mat must have exactly four channels");
        if (status != OPENCV_CORE_OK) return status;
        // ABI safety: at<T> dereferences an unchecked typed element address.
        if (mat->value.data == nullptr ||
            mat->value.elemSize() != sizeof(cv::Vec<T, 4>))
            return invalid_argument("Mat has no matching Vec4 storage");
        const auto &v = mat->value.at<cv::Vec<T, 4>>(row, column);
        *out = {v[0], v[1], v[2], v[3]};
        return OPENCV_CORE_OK;
    } catch (...) { return translate_current_exception(); }
}
...
template <typename T, typename Abi>
opencv_core_status vec4_set(opencv_core_mat_handle *mat, int32_t row,
                            int32_t column, const Abi *value, int depth,
                            const char *message) {
    clear_error();
    if (value == nullptr) return invalid_argument("value must not be null");
    ...
    try {
        ...
        if (mat->value.data == nullptr ||
            mat->value.elemSize() != sizeof(cv::Vec<T, 4>))
            return invalid_argument("Mat has no matching Vec4 storage");
        assign_vec(mat->value.at<cv::Vec<T, 4>>(row, column),
                   cv::Vec<T, 4>(value->component_0, value->component_1,
                                 value->component_2, value->component_3));
        return OPENCV_CORE_OK;
    } catch (...) { return translate_current_exception(); }
}

template <typename T, typename Abi>
opencv_core_status vec4_get_nd(const opencv_core_mat_handle *mat, int32_t ndims,
                               const int32_t *indices, Abi *out, int depth,
                               const char *message) {
    clear_error();
    if (out == nullptr) return invalid_argument("out_value must not be null");
    *out = {0, 0, 0, 0};
    ...
    try {
        int converted[maximum_mat_dimensions];
        const auto status = prepare_nd_vector_at(
            mat->value, ndims, indices, converted, depth, 4,
            sizeof(cv::Vec<T, 4>), message,
            "Mat must have exactly four channels");
        if (status != OPENCV_CORE_OK) return status;
        const auto &v = mat->value.at<cv::Vec<T, 4>>(converted);
        *out = {v[0], v[1], v[2], v[3]};
        return OPENCV_CORE_OK;
    } catch (...) { return translate_current_exception(); }
}
...
template <typename T, typename Abi>
opencv_core_status vec4_set_nd(opencv_core_mat_handle *mat, int32_t ndims,
                               const int32_t *indices, const Abi *value,
                               int depth, const char *message) {
    clear_error();
    if (value == nullptr) return invalid_argument("value must not be null");
    ...
    try {
        ...
        const auto status = prepare_nd_vector_at(
            ...
            "Mat must have exactly four channels");
        if (status != OPENCV_CORE_OK) return status;
        assign_vec(mat->value.at<cv::Vec<T, 4>>(converted),
                   cv::Vec<T, 4>(value->component_0, value->component_1,
                                 value->component_2, value->component_3));
        return OPENCV_CORE_OK;
    } catch (...) { return translate_current_exception(); }
...
opencv_core_mat_read_float64_row(const opencv_core_mat_handle *mat,
    ...
    try {
        const double *row_data = nullptr;
        std::size_t byte_count = 0;
        const opencv_core_status status =
            prepare_row(mat, row, element_count, CV_64F,
                        "Mat depth must be Float64", row_data, byte_count);
        if (status != OPENCV_CORE_OK) {
            return status;
        }
...
opencv_core_status
opencv_core_mat_write_float64_row(opencv_core_mat_handle *mat, int32_t row,
                                  const double *data,
                                  uint64_t element_count) {
    clear_error();
    ...
    try {
        ...
        if (byte_count != 0) {
            std::memcpy(const_cast<double *>(row_data), data, byte_count);
        }
        return OPENCV_CORE_OK;
    } catch (...) {
        return translate_current_exception();
...
opencv_core_mat_read_float16_row(const opencv_core_mat_handle *mat,
    ...
    try {
        ...
        std::size_t byte_count = 0;
        const opencv_core_status status =
            prepare_float16_row(mat, row, element_count, row_data, byte_count);
        if (status != OPENCV_CORE_OK) {
            return status;
        }
...
opencv_core_mat_dft(const opencv_core_mat_handle *source,
    ...
    case OPENCV_CORE_DFT_INVERSE_REAL:
        ...
        break;
    case OPENCV_CORE_DFT_ROWS_FORWARD_COMPLEX:
        flags = cv::DFT_ROWS;
        if (source->value.channels() == 1) {
            flags |= cv::DFT_COMPLEX_OUTPUT;
        }
        break;
    case OPENCV_CORE_DFT_ROWS_INVERSE_COMPLEX:
        flags = cv::DFT_ROWS | cv::DFT_INVERSE | cv::DFT_SCALE;
        break;
    case OPENCV_CORE_DFT_ROWS_INVERSE_REAL:
        flags = cv::DFT_ROWS | cv::DFT_INVERSE | cv::DFT_SCALE |
                cv::DFT_REAL_OUTPUT;
        break;
    ...
    case OPENCV_CORE_DFT_INVERSE_PACKED:
        flags = cv::DFT_INVERSE | cv::DFT_SCALE | cv::DFT_REAL_OUTPUT;
        break;
    case OPENCV_CORE_DFT_ROWS_FORWARD_PACKED:
        flags = cv::DFT_ROWS;
        break;
    case OPENCV_CORE_DFT_ROWS_INVERSE_PACKED:
        flags = cv::DFT_ROWS | cv::DFT_INVERSE | cv::DFT_SCALE |
                cv::DFT_REAL_OUTPUT;
        break;
    default:
...
opencv_core_mat_extract_channel(const opencv_core_mat_handle *source,
    ...
    *out_mat = nullptr;
    ...
    try {
        // OpenCV compatibility: OpenCV 5 extractChannel output creation can
        // construct a 0-D scalar destination for a default-constructed source
        // (dims == 0) rather than the expected empty matrix. After Ada channel
        // validation, a default-empty source produces an independently owned
        // empty result with the source depth and one channel. Typed 0x0 Mats
