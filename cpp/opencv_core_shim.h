...
/* Draws cv::RNG::next() from caller-owned state. */
opencv_core_status
opencv_core_rng_next(uint64_t *rng_state, uint32_t *out_value);

/* Draws cv::RNG::uniform(double, double) from caller-owned state. */
opencv_core_status
...
#define OPENCV_CORE_SPECTRUM_REPRESENTATION_PACKED_CCS_ROWS ((int32_t)2)
 ...
 * explicit row step. byte_count is the accessible capacity beginning at data
 * and must cover rows * row_stride_bytes. OpenCV sets
 * datalimit = datastart + step * rows, so padding after the final logical
 * row is part of the header extent and must exist. row_stride_bytes is the
 ...
 * allocation size; the caller must supply a truthful capacity and live storage.
 */
opencv_core_status
...
/* Fills an existing Mat in place using the caller-owned cv::RNG state. */
opencv_core_status
...
/* Shuffles using the caller-owned cv::RNG state. */
opencv_core_status
...
/*
 * Multiplies two borrowed spectra using cv::mulSpectrums. representation must
 * explicitly identify full-complex, ordinary packed CCS, or row-wise packed CCS
 * inputs. kind must be one of the OPENCV_CORE_SPECTRUM_PRODUCT_*
 * identifiers; the shim maps it to conjB and never forwards a raw OpenCV flag
 * mask. Row-wise packed CCS maps to DFT_ROWS; the other representations map to
 * flags 0. Matrix representation requirements are validated by the thick Ada
 * API.
 *
 * ORDINARY:
